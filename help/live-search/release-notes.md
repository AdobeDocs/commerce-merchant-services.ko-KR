...
이러한 릴리스 노트에서는 최신 버전의 [!DNL Live Search].
...
![새로 만들기](../assets/new.svg) 이제 라이브 검색 고객이 새로운 기능을 활용할 수 있습니다 [SaaS 가격 인덱서](../price-index/index.md).
...
* [카테고리 필터링 선택 사항](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search/#facets)
* 드래그 앤 드롭하여 Pin 이벤트를 만드는 기능을 추가했습니다
* 새 고정 작업:
   * 스팟에 고정 - 한 번의 클릭으로 고정 이벤트를 만들 수 있는 고정 단추
   * 맨 위에 고정 - 첫 번째 위치에 제품 위치
   * 맨 아래에 고정 - 제품 결과를 맨 아래에 배치합니다
   * 한 번의 클릭으로 이벤트 고정 해제
* [규칙에 대한 지능형 등급](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/live-search-admin/rules/rules-add.html#ranking-type)

### 업데이트

* 이제 규칙 구성 은 Position을 고유하게 자동 정렬합니다
* 이제 기존 이벤트를 삭제하면 미리 보기가 업데이트됩니다
* 이벤트가 없는 규칙은 저장할 수 있습니다
* &quot;유형 선택&quot; 선택기 제거
* 저장하지 않은 규칙에 대한 새 &quot;편집&quot; 상태가 추가되었습니다

### 수정 사항

* 저장하는 동안 완료되지 않은 이벤트가 있을 때 서버 오류가 해결되었습니다.
* 여러 이벤트가 있을 때 특정 이벤트를 올바르게 삭제하는 문제를 해결했습니다.
* 새 이벤트가 추가되면 기존 규칙 이벤트가 업데이트되지 않는 문제를 수정했습니다
* 세부 정보에서 두 번째 &quot;편집&quot; 클릭이 수정되었습니다. [!DNL Live Search] 다시 로드해야 하는 페이지
* 동의어: 사용자가 입력을 초과했을 때 포커스를 필드에 반환할 수 없는 문제를 수정했습니다
* 기타 사소한 버그 수정 및 성능 업데이트


* ![버그](../assets/bug.svg) - &quot;권장됨&quot;의 등급은 라이브 검색 위젯 내에서만 지원됩니다. 기본 Luma 및 PWA 검색 기능에서는 지원되지 않습니다.
* ![버그](../assets/bug.svg) - 사용자 지정 가격 속성 패싯이 Luma에서 올바르게 렌더링되지 않지만, API가 이 패싯에서 제대로 필터링합니다.

상인들은 [!DNL Live Search] 확장 버전 >= 3.0.1 을 사용하여 이러한 기능에 액세스합니다.
...
* ![수정](../assets/fix.svg) - 네트워크 문제로 인해 SDK 리소스를 사용할 수 없을 때 라이브 검색에서 오류가 발생합니다. 이 버그가 수정되었습니다.
...
상인들은 [!DNL Live Search] 확장 버전 >= 2.0.4 을 사용하여 이러한 기능에 액세스합니다.
...
상인들은 [!DNL Live Search] 확장 버전 >= 2.0.3 을 사용하여 이러한 기능에 액세스합니다.
...
![버그](../assets/bug.svg) [!DNL Live Search] 동의어는 스토어 보기별로 정의되지만, 현재 웹 사이트별로 저장되며, `environmentId` 및 `storeViewCode`. 따라서 Adobe Commerce 설치 내의 모든 웹 사이트 및 저장소 보기에서는 동의어가 공유됩니다. 저장소 보기에 대해 가장 최근에 만든 동의어 세트가 우선합니다.
