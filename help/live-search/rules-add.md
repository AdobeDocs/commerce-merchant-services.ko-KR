...
규칙에는 최대 10개의 조건이 있을 수 있습니다. 두 조건을 결합하는 논리 연산자는 현재 조건을 기반으로 합니다 *일치* 설정. 기본적으로, *일치* 은(는) `All` 논리 연산자는 `AND`.
   ...
   ![규칙 - 검색 쿼리에 다음이 포함됩니다.](assets/rules-search-query-contains-and.png)
...
순위는 사용자 행동과 사이트 통계를 결합하여 제품 순위를 결정합니다.
...
이벤트는 정의된 조건이 충족될 때 검색 결과를 수정하는 작업입니다. 단일 규칙에는 최대 25개의 이벤트가 포함될 수 있습니다.
...
1. 개요 입력 **설명** 규칙.
1. 다음을 입력합니다. **시작일** 및 **종료일** 을 눌러 규칙을 활성화하거나 달력에서 날짜를 선택합니다.
