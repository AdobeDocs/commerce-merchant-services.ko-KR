...
[!DNL Live Search] 는 Adobe Marketplace에서 확장으로 설치됩니다. 다음 이후 [!DNL Live Search] 모듈(카탈로그 모듈을 종속성으로 사용)이 설치 및 구성되고 [!DNL Commerce] SaaS 서비스와 검색 및 카탈로그 데이터 공유를 시작합니다. 이 시점에서 *관리* 사용자는 검색 패싯, 동의어 및 머천다이징 규칙을 설정, 사용자 지정 및 관리할 수 있습니다.
...
>[!NOTE]
>
>2023년 3월 현재 라이브 검색에서는 버전 2.4.4 이상만 지원합니다.
...
>[!IMPORTANT]
>
>2023년 8월에 대한 Elasticsearch 7 지원 종료 발표로 인해 모든 Adobe Commerce 고객은 OpenSearch 2.x 검색 엔진으로 마이그레이션하는 것이 좋습니다. 제품 업그레이드 중 검색 엔진 마이그레이션에 대한 자세한 내용은 [OpenSearch로 마이그레이션](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/prepare/opensearch-migration.html) 에서 _업그레이드 안내서_.
...
2.0.0에서 3.0.1과 같은 주요 버전으로 업데이트하려면 프로젝트의 루트를 편집하십시오 [!DNL Composer] `.json` 파일:

1. 현재 설치되어 있는 경우 `magento/live-search` version `2.0.3` 또는 아래에 있는 를 버전으로 업그레이드할 수 있습니다 `3.0.0` 업그레이드 전에 다음 명령을 실행하십시오.
...
| `data-services` | Commerce Services에 대한 연결을 구성하는 데 필요합니다. |
| `services-id` | Commerce Services에 대한 연결을 구성하는 데 필요합니다. |
