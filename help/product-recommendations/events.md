...
`magento/product-recommendations` 모듈을 설치하면 Adobe Sensei에서 동작 및 카탈로그 데이터를 집계하여 각 권장 사항 유형에 대한 제품 Recommendations을 만듭니다. 그런 다음 Product Recommendations 서비스는 이러한 권장 사항을 권장 제품 _items_&#x200B;을(를) 포함하는 위젯 형태로 상점 앞에 배포합니다.
...
>[!NOTE]
>
>제품 추천 지표는 Luma 상점 전선에 최적화되어 있습니다. Storefront가 PWA Studio으로 구현된 경우 [PWA 설명서](https://developer.adobe.com/commerce/pwa-studio/integrations/product-recommendations/)를 참조하세요. React 또는 Vue JS와 같은 사용자 지정 프론트엔드 기술을 사용하는 경우 Headless](headless.md) 환경에서 [제품 Recommendations을 통합하는 방법을 알아보세요.
...
- [Headless 구현](headless.md)은(는) 제품 Recommendations 대시보드를 구동하는 이벤트를 구현해야 합니다.
