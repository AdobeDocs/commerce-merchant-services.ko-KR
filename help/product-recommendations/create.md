...
1. 권장 사항을 표시할 [스토어 보기](https://experienceleague.adobe.com/en/docs/commerce-admin/start/setup/websites-stores-views)를 지정하십시오.
   ...
   >[!NOTE]
   >
   > Page Builder 권장 사항 단위는 기본 스토어 보기에서 만들어야 하지만 어디에서나 사용할 수 있습니다. Page Builder를 사용하여 제품 권장 사항을 만드는 방법에 대한 자세한 내용은 [콘텐츠 추가 - 제품 Recommendations](https://experienceleague.adobe.com/en/docs/commerce-admin/page-builder/add-content/recommendations)을 참조하세요.
...
1. _페이지 유형 선택_ 섹션에서 다음 옵션 중 권장 사항을 표시할 페이지를 선택합니다.
   ...
   >[!NOTE]
   >
   > 장바구니에 제품을 추가한 후 [장바구니 페이지를 바로 표시](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/point-of-purchase/cart/cart-configuration)하도록 스토어를 구성한 경우 장바구니 페이지에서 제품 Recommendations이 지원되지 않습니다.
...
준비 지표는 사용 가능한 카탈로그 및 동작 데이터를 기반으로 어떤 권장 사항 유형이 가장 잘 수행되는지 보여 줍니다. 또한 준비 표시기를 사용하여 이벤트에 문제가 있는지 또는 추천 유형을 채울 트래픽이 충분하지 않은지 확인할 수도 있습니다.

준비 표시기는 [정적 기반](#static-based) 또는 [동적 기반](#dynamic-based) 중 하나로 분류됩니다. 정적 기반은 카탈로그 데이터만 사용하는 반면, 동적 기반은 쇼핑객의 행동 데이터를 사용합니다. 해당 동작 데이터는 [기계 학습 모델 학습](behavioral-data.md)을 통해 개인화된 추천을 작성하고 준비 점수를 계산하는 데 사용됩니다.
...
* 50% 미만은 해당 권장 사항 유형에 대해 제안된 권장 사항이 관련이 없음을 의미합니다.

이는 일반적인 지침이지만 위에 요약된 대로 수집된 데이터의 특성에 따라 각 개별 사례가 다를 수 있습니다. [준비 지표를 계산하는 방법](#understand-how-readiness-indicators-are-calculated) 및 [준비 지표가 낮은 이유](#what-to-do-if-the-readiness-indicator-percent-is-low)에 대해 자세히 알아보세요.
...
준비 비율이 낮으면 이 권장 사항 유형에 대한 권장 사항에 포함할 수 있는 카탈로그의 제품이 많지 않음을 나타냅니다. 즉, 이 권장 사항 유형을 배포하면 [백업 권장 사항](behavioral-data.md#backuprecs)이 반환될 가능성이 높습니다.
...
* **동적 기반** - 동적 기반 지표에 대한 낮은 비율은 다음 원인으로 인해 발생할 수 있습니다.
   ...
   * 각 권장 사항 유형(requestId, 제품 컨텍스트 등)에 필요한 상점 첫 번째 이벤트에 필드가 누락되었습니다.
...
준비 지표는 모델이 얼마나 훈련되는지를 나타냅니다. 지표는 수집된 이벤트의 유형, 상호 작용하는 제품의 범위 및 카탈로그의 크기에 독립적입니다.
