...
구성 [!DNL Store Fulfillment] 다음에서 [!DNL Commerce] 관리자가 확장을 활성화하고, 확장 설정을 지정하고, Store Assist 앱 사용자에 대한 보안 설정을 구성하고, 배달 방법에 대한 옵션을 설정합니다.
...
<td>Walmart Store Fulfillment API 엔드포인트에 대한 URL. 온보딩 프로세스 중에 제공되는 정규화된 URL이어야 합니다. 스토어 이행 고객은 샌드박스와 프로덕션 URL을 모두 받습니다. 값을 추가할 때 슬래시 "/"를 포함하여 전체 URL을 복사하여 붙여넣어야 합니다.</td>
<td>글로벌</td>
<td>예</td>
</tr>
<tr>
...
<td>심각한 오류가 발생한 후 레코드 동기화 작업을 다시 시도하도록 지정합니다.<br></br>통합이 이행 서비스에서 긍정적인 응답을 얻지 못할 때마다 심각한 오류가 발생합니다. 이 문제는 서비스가 중단되거나 전송되는 주문 데이터에 오류가 있을 때 발생할 수 있습니다.<br></br>다시 시도 임계값에 도달하면 항목이 큐에 남아 있지만 다시 처리되지 않습니다. 다음에서 오류가 발생한 모든 항목 보기: <strong>[!UICONTROL System > Tools > Store Fulfillment Queue]</strong> 관리에서의 관리. 지속적으로 오류가 발생하는 항목의 문제를 해결하려면 계정 관리자에게 문의하십시오.</td>
<td>글로벌</td>
<td>아니요</td>
</tr>
<tr>
...
<td>판매자 위치의 해당 항목에 대한 훑어볼 수 있는 코드를 저장하는 카탈로그 속성.<br></br>기존 판매자 위치가 한 곳뿐인 경우 전자 상거래 채널이 SKU별로 제품을 식별하는 동안 UPC 코드를 사용할 수 있습니다. 시나리오인 경우 UPC 코드가 포함된 카탈로그 속성을 선택합니다.<br></br>이 설정을 사용하면 스토어로 보낸 주문에서 올바른 식별자를 사용하여 목록 항목을 검색할 수 있으므로 스토어 관계자가 피킹 프로세스 중에 항목을 정확하게 스캔할 수 있습니다.<br></br>확실하지 않은 경우 운송 및 피킹 부서의 이행 담당자에게 문의하여 어떤 속성을 전송해야 하는지 확인하십시오. 속성이 현재 데이터베이스에 포함되어 있지 않은 경우 Adobe Commerce 제품 속성 집합에 적절한 속성을 추가해야 할 수 있습니다.</td>
<td>웹 사이트</td>
<td>예</td>
</tr>
<tr>
...
<td>저장소 이행 대기열에서 한 번에 보낼 최대 항목 수입니다.<br></br>BOPIS 주문은 일정한 간격으로 이행 서비스로 일괄적으로 전송됩니다. 이 설정을 사용하면 일괄 처리의 크기를 제어할 수 있습니다.<br></br>기본값은 100개 항목입니다. 주문 수량 및 용량에 따라 이 값을 위 또는 아래로 조정해야 할 수 있습니다.</td>
<td>글로벌</td>
<td>아니요</td>
</tr>
</tbody>
...
<td>납품처 스토어 설정은 기존 납품처 스토어 기능을 기반으로 합니다. Inventory management을 사용하거나, 점포 간 재고 이전을 통해 재고가 없는 판매자 위치에서 주문을 수락하고 이행할 수 있는 경우, 이 옵션을 '예'로 설정합니다.<br></br>납품처 스토어 옵션을 지원할 수 없거나 제공을 원하지 않는 경우 '아니요'로 설정합니다. 비활성화되면, 판매점에 대한 재고가 없는 카탈로그의 품목 또는 해당 위치 아래에 있는 품목 [!DNL Out of Stock Threshold]은 매장 내 픽업 옵션과 함께 제공되지 않습니다.<br></br>가맹점 위치별로 조정할 수 있는 글로벌 설정입니다.</td>
<td>글로벌</td>
<td>아니요</td>
</tr>
</tbody>
...
<td>가맹점에서 택배 옵션을 활성화하거나 비활성화합니다. 활성화하면 가맹점 위치는 웹 사이트와 연계된 재고에서 지정된 다른 출처와 합산되어 고려됩니다.<br></br>표준 Inventory management 서비스에서 [!DNL Ship from Store] is 옵션은 고유하며 비활성화할 수 없습니다. Store Fulfillment 솔루션을 사용하면 이 기능을 켜거나 끌 수 있습니다.<br></br>전역 설정입니다. 판매자 위치 및 제품별로 이 설정을 조정할 수도 있습니다.</td>
<td>글로벌</td>
<td>아니요</td>
</tr>
</tbody>
...
- **매장 픽업**- 체크아웃 프로세스 동안 매장 게재를 위한 오퍼 옵션 이 옵션은 BOPIS 주문에 대한 가장 일반적인 게재 시나리오입니다.
...
<td>소매 상점에서 주문이 수거될 준비가 되면 고객에게 이메일로 통지됩니다. 고객이 선택한 경우 [!DNL In-Store Pickup] 체크아웃하는 동안 여기에서 픽업 지침을 사용자 지정할 수 있습니다. </br></br>모든 소매점 위치에 적용되는 글로벌 설정입니다. 소매점 위치 수준에서 지침을 사용자 지정할 수도 있습니다.</td>
<td>스토어 뷰</td>
<td>아니요</td>
</tr>
<tr>
<td><strong>커브사이드 픽업 지침</strong></td>
<td>커브사이드 픽업 주문에 대한 고객 이메일 알림에 포함할 사용자 지정 주문 픽업 지침을 지정합니다. </br></br>모든 소매점 위치에 적용되는 글로벌 설정입니다. 소매점 위치 수준에서 지침을 사용자 지정할 수도 있습니다.</td>
<td>스토어 뷰</td>
<td>아니요</td>
</tr>
<tr>
<td><strong>예상 픽업 리드 타임</strong></td>
<td>주문이 접수, 이행되고 픽업될 때까지 필요한 시간(분)입니다. 이 정보는 매장 픽업 배송 옵션에 대한 소매점 위치를 선택할 때 고객에게 표시됩니다. 이는 글로벌 설정이며 모든 소매점 위치에 적용됩니다. 소매점 위치 수준에서 리드 타임을 사용자 지정할 수도 있습니다.</td>
<td>스토어 뷰</td>
<td>아니요</td>
</tr>
<tr>
<td><strong>예상 픽업 시간 레이블</strong></td>
<td>고객 픽업에 주문을 사용할 수 있을 때까지의 예상 시간을 표시합니다. 이 정보는 고객이 다음에 대한 소매점 위치를 선택할 때 표시됩니다 [!DNL In-Store Pickup] 게재 옵션. </br></br>이 레이블을 사용자 지정할 때 코드를 사용할 수 있습니다 <code>%1</code> 을(를) 삽입하려면 <strong>예상 픽업 리드 타임</strong>. 예:</br></br><code>Ready for Pickup in %1 minutes.</code></br></br>모든 소매점 위치에 적용되는 글로벌 설정입니다. 소매점 위치 수준에서 리드 타임을 사용자 지정할 수도 있습니다.</br></br><code>Ready for Pickup in %1 minutes.</code></br></br></td>
<td>스토어 뷰</td>
<td>아니요</td>
<tr>
...
<td>고객이 소매점 보관처를 사용하는 경우 각 위치에 대해 현재 품목의 재고 가용성이 표시됩니다. </br></br>다음을 사용자 지정할 수 있습니다. <em>[!UICONTROL in-stock]</em> 여기에 상태 레이블이 있습니다.</br></br></td>
<td>스토어 뷰</td>
<td>아니요</td>
</tr>
<tr>
...
<td>고객이 소매점 보관처를 사용하는 경우 각 위치에 대해 현재 품목의 재고 가용성이 표시됩니다. </br></br>다음을 사용자 지정할 수 있습니다. <em>[!UICONTROL partially in-stock]</em> 여기에 상태 레이블이 있습니다.</br></br></td>
<td>스토어 뷰</td>
<td>아니요</td>
</tr>
</tbody></table>
