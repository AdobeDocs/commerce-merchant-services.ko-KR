...
* **[주문 결제 상태 시각화 보기](#order-payment-status-data-visualization-view)**- 지급 서비스 홈에서 사용할 수 있는 차트로, 주문 지급 상태 보고서 보기에서 일별 종합 지급 상태를 시각적으로 나타낸 것입니다.
* **[주문 결제 상태 보고서 보기](#order-payment-status-report-view)**—모든 거래에 대한 자세한 지급, 송장 발행, 배송, 환불 및 분쟁 상태를 보여 주는 주문 결제 상태에서 사용할 수 있는 보고서
...
다음을 수행할 수 있습니다. [지급 거래 다운로드](#download-order-payment-statuses) 기존 회계 또는 order management 소프트웨어에서 사용할 .csv 파일 형식으로.
