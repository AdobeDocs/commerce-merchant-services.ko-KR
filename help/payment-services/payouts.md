...
* **[지급액 데이터 시각화 보기](#payouts-data-visualization-view)**- 지급 서비스 홈에서 사용할 수 있는 차트로, 지급 보고서 보기에서 일별 합산된 금액을 시각적으로 표시한 것입니다.
* **[지급 보고서 보기](#payouts-report-view)**- 모든 거래에 대한 자세한 지급 정보를 표시하는 지급액에서 사용할 수 있는 보고서

지급 조회는 종합적인 지급 정보를 한눈에 표시하므로, 지급 금액, 처리 수량 및 재무 조정을 위한 거래 수준에 대한 상세한 보고를 완전히 투명하게 확인할 수 있습니다.

>[!NOTE]
>
>지급 보고서는 수집된 주문만 표시합니다(지급 조치가 다음으로 설정됨). [`Authorize and Capture`](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/get-started/production.html#set-payment-services-as-payment-method)) - 또는 [다음으로 표시: `Invoiced`](https://docs.magento.com/user-guide/sales/invoice-create.html).

## 지급액 데이터 시각화 보기

지급 데이터 시각화 보기는 결제 서비스 홈에서 사용할 수 있습니다. 이는 세부 테이블 형식으로 일별 집계된 금액을 시각적으로 나타낸 것입니다 [지급 보고서 보기](#payouts-report-view).
...
1. 다음에서 _관리자_ 사이드바, 이동 **[!UICONTROL Sales]** > **[!UICONTROL Payment Services]**. 지급액 데이터 시각화 보기가 지급액 섹션에 표시됩니다.
...
지급 보고서 보기는 지급 서비스의 지급 보기에서 사용할 수 있습니다. 여기에는 스토어의 지불에 대해 사용 가능한 모든 정보가 포함됩니다. 다음 [지급액 데이터 시각화 보기](#payouts-data-visualization-view) 지급 서비스 홈은 이 더 상세한 보고서 보기에서 일별 합산 금액을 시각적으로 표시한 것입니다.
...
* 라이브 모드에 (하나 또는 여러 개) 저장소가 있는 경우 데이터 소스 선택 기본값은 입니다. _[!UICONTROL Live]_.
* 보고서 내보내기는 항상 데이터 소스 선택을 따릅니다.

주문 지급 상태 보고서에 대한 데이터 출처를 선택하려면

1. 다음에서 _관리자_ 사이드바, 이동 **[!UICONTROL Sales]** > **[!UICONTROL Payment Services]** > **[!UICONTROL Payouts]**.
...
왼쪽과 오른쪽으로 스크롤하여 보기 [각 지급 거래에 대한 정보](#column-descriptions) 트랜잭션 일자, 참조 ID, 송장 번호 및 결제 방법 세부 정보가 포함된 일별 보고서에서.
...
1. 다음에서 _관리자_ 사이드바, 이동 **[!UICONTROL Sales]** > **[!UICONTROL [!DNL Payment Services]]** > **[!UICONTROL Payouts]**.
